...
```{r}
library(dplyr)
turtles_yearbybeach<-merge(turtles1,turtles2,all.x=T,all.y=T)
turtles_yearbybeach<-arrange(turtles_yearbybeach,unique_id)
turtles_yearbybeach[1:5,]
```
...
```{r}
turtles_yearbybeach<-turtles_yearbybeach[turtles_yearbybeach$unique_id!="798",]
turtles_yearbybeach<-droplevels(turtles_yearbybeach,"798")
...
```{r}
library(dplyr)
turtles5<-as.data.frame(turtles %>% count(unique_id,beach, year, sort = TRUE))
turtles5$year<-as.numeric(turtles5$year)
turtles5<-arrange(turtles5,desc(unique_id))
turtles5<-turtles5[turtles5$unique_id!="798",]
turtles5<-droplevels(turtles5,"798")
turtles5[1:5,]
```
...
```{r}
library(dplyr)
turtles_yearbybeach<-mutate(turtles_yearbybeach,s_to_e=rep(NA))
turtles_yearbybeach<-mutate(turtles_yearbybeach,e_to_s=rep(NA))
turtles_yearbybeach<-mutate(turtles_yearbybeach,s_to_s=rep(NA))
turtles_yearbybeach<-mutate(turtles_yearbybeach,e_to_e=rep(NA))
```
...
```{r}
library(data.table)
turtles_yearbybeach<-as.data.table(turtles_yearbybeach)
turtles_yearbybeach[,id_rank:=rank(date,ties.method="first"),by=unique_id]
turtles_yearbybeach<-as.data.frame(turtles_yearbybeach)
```
